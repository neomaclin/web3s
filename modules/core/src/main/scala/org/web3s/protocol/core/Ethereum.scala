...
trait Ethereum[F[_]] {
  ...
  def ethGetStorageAt(address: String, position: BigInteger, defaultBlockParameter: DefaultBlockParameter): F[EthGetStorageAt]
  ...
  def ethGetTransactionByBlockHashAndIndex(blockHash: String, transactionIndex: BigInteger): F[EthTransaction]

  def ethGetTransactionByBlockNumberAndIndex(defaultBlockParameter: DefaultBlockParameter, transactionIndex: BigInteger): F[EthTransaction]

  def ethGetTransactionReceipt(transactionHash: String): F[EthGetTransactionReceipt]

  def ethGetUncleByBlockHashAndIndex(blockHash: String, transactionIndex: BigInteger): F[EthBlock]

  def ethGetUncleByBlockNumberAndIndex(defaultBlockParameter: DefaultBlockParameter, transactionIndex: BigInteger): F[EthBlock]
  ...
  def ethUninstallFilter(filterId: BigInteger): F[EthUninstallFilter]

  def ethGetFilterChanges(filterId: BigInteger): F[EthLog]

  def ethGetFilterLogs(filterId: BigInteger): F[EthLog]
...
//
//  def shhUninstallFilter(filterId: BigInteger): F[ShhUninstallFilter]
//
//  def shhGetFilterChanges(filterId: BigInteger): F[ShhMessages]
//
//  def shhGetMessages(filterId: BigInteger): F[ShhMessages]
