...
object NumericType:
  ...
  def encode(numericType: NumericType): String =
    def toByteArray(numericType: NumericType): Array[Byte] =
      numericType match
        case i:SolidityUInt => if i.bitSize == MAX_BIT_LENGTH then i.value.toByteArray.tail else i.value.toByteArray
        case u:UFixed => if u.bitSize == MAX_BIT_LENGTH then u.value.toByteArray.tail else u.value.toByteArray
        case _ => numericType.value.toByteArray
    end toByteArray
  ...
  // def decode(input: String): NumericType =
